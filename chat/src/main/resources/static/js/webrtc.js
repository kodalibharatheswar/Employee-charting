...
subscribeToSignaling() {
  ...
  subscribeToGroupCall(chatRoomId) {
    ...
    this.stompClient.subscribe(
      `/topic/chatroom.${chatRoomId}.signal`,
      (message) => {
        this.handleSignalingMessage(JSON.parse(message.body));
      },
    );
    ...
    this.stompClient.subscribe(
      `/topic/chatroom.${chatRoomId}.call`,
      (message) => {
        this.handleCallNotification(JSON.parse(message.body));
      },
    );
...
async startCall(chatType, chatId, callType) {
  ...
  async createPeerConnection(userId) {
    try {
      ...
      peerConnection.onconnectionstatechange = () => {
        console.log(
          `Connection state for user ${userId}:`,
  ...
  async createAndSendOffer(userId) {
    try {
      ...
      const offer = await peerConnection.createOffer({
        ...
        offerToReceiveVideo:
          this.callType === "VIDEO" || this.callType === "SCREEN_SHARE",
      });
  ...
  async handleIceCandidate(data) {
    try {
      ...
      if (!peerConnection) {
        console.warn(
          "No peer connection found for ICE candidate from user",
          userId,
        );
        return;
      }
